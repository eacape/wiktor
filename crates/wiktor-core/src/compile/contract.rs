...
2. 每个引用 ref 的 id 形如 r1、r2（递增，全页唯一），entity_id 与 source_revision 必须等于知识快照的值；pointer 是 RFC6901 指针，只能指向 /fields/<字段> 下的字符串（数组字段必须带下标，如 /fields/ingredients/0）；value 必须等于指针解出的原值；quote 必须是该原值的连续精确子串。
