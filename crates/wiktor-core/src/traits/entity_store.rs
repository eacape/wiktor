...
/// Fact-plane storage (SQLite implementation is provided by the kernel).
#[async_trait]
pub trait EntityStore: Send + Sync {
    /// 幂等写入事实；`source_revision` 用于 CAS——旧版本不得覆盖新版本。
    /// Idempotently writes facts; `source_revision` provides CAS so old revisions cannot overwrite new ones.
    async fn upsert_facts(&self, id: &EntityId, facts: &Facts, source_revision: u64) -> Result<()>;
    /// 事实过滤下推：返回候选实体 ID 集合（供向量检索预筛）。
    /// Fact filter pushdown: returns candidate entity IDs for vector pre-filtering.
    async fn filter(&self, filters: &Filters) -> Result<Vec<EntityId>>;
    /// 删除事实（tombstone 传播）。
    /// Deletes facts (propagating the tombstone).
    async fn delete_facts(&self, id: &EntityId) -> Result<()>;
    /// 读取单实体事实。
    /// Reads facts for one entity.
