...
- **`code.html` is a purely real-data UI (Step12 B2 + the STEP12-005 redesign)**: four tabs (Overview / Compile Tasks / Review Queue / Search) render live `/api/*` data (15s poll) with zero mock content; when the API is unreachable an offline badge appears and the last known data stays; English is the default language with a one-click 中文 toggle (persisted in localStorage); single file, no build step, no CDN dependencies.
