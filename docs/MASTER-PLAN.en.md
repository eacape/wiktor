...
12. TUI (clearing the Step 11 debt) + console frontend on real data + production deployment orchestration ✅ (Step 12 delivered 2026-09-25; ①`wiktor tui` (feature `tui`, ratatui 0.29 + crossterm 0.28): four tabs (dashboard/tasks/reviews/query) sharing the Web console's data plane (in-process kernel reads + the QueryEngine hybrid fallback), pure state functions + TestBackend tests, keyboard navigation (1-4/Tab/Enter/q); ②the console frontend on real data: `/api/overview` gains due-task status counts, `code.html` carries `data-live` markers + a vanilla-fetch rendering layer (15s poll, offline-badge fallback, the search box hitting `POST /api/search` to render hits and the fts/vector/rrf_k breakdown), browser-verified with a real seeded DB PASS; ③production deployment orchestration in `deploy/`: `wiktor-server`/`wiktor-console` systemd units (sandbox aligned with litestream, loopback listeners) + `wiktor.env.example` (WIKTOR_API_KEYS) + `install-wiktor.sh` (release build on the server, idempotent) + `smoke-deploy.sh` + a bring-up runbook, accepted on a real Linux Debian 13 box; precondition: the rsproxy mirror unlocked crates.io (root cause of STEP11-001/005 resolved); deviations in `docs/design/step12-tui-console-prod(.en).md` §7 (STEP12-001..005; 005 = the user-directed rewrite of code.html from the reference prototype into a purely real-data supervisory UI — rendering only live /api/* data with zero mock content, English-default with a Chinese toggle, single-file with zero external dependencies))

13. Production readiness (real serve retrieval + backup replica + observability/CI + feedback-loop E2E) ✅ (Step 13 delivered 2026-09-25; ①real serve retrieval: `wiktor serve` injects the vector backend/embedder/QUG from the CLI assembler driven by env (`WIKTOR_VECTOR_BACKEND=mock|qdrant` + `WIKTOR_QDRANT_URL/API_KEY` + `WIKTOR_EMBEDDING_*`; mock default runs offline; dimension-probe assembly), and HTTP `GET /search` switches to the same `QueryEngine<dyn VectorStore>` as gRPC — the response shape is unchanged while diagnostics/log_id go from placeholder to real and land in query_logs; ②console placeholder debt cleared: kernel read-only `quality_summary()`/`list_domains()` + real `/api/quality` `/api/domains` + a quality panel on the overview page; ③backup replica: the litestream file:// replica is live in production (`/srv/backup/wiktor/litestream`, sync-interval 1s, status ok) and the isolated restore drill against the production DB PASSed (integrity/fk/per-table row parity); ④observability + CI: Prometheus scrapes `/metrics` (target up) + an `up==0` alert rule + GitHub Actions (fmt/clippy -D warnings/test incl. the tui feature); ⑤feedback-loop E2E: `deploy/smoke-feedback.sh` PASSes locally and in production — search (log_id) → POST /feedback → `feedback analyze --domain-pack` (the zero-recall blind spot + deterministic entity enrichment) → review approve → the worker compiles and publishes → re-check hits 0→1; deviations in `docs/design/step13-production-ready(.en).md` §7 (STEP13-001..003; 001 = the optional analyze-time entity enrichment bridging the step6-spec gap between the report-shaped subject and approve's five-field validation))
