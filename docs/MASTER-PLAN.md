...
- **退出条件（v3.1 新增；口径 v3.2-P2 统一）**：若 QUG 在 golden-queries 评测集上无显著增益，QUG 模块默认关闭、停在混合检索，不作为阻塞交付。**口径以绝对百分点（pp）为准**：`gain_pp = (C_recall@10 − B_recall@10) × 100 ≥ 5.0pp` 才判定启用（代码 `QUG_ENABLE_GAIN_PP_THRESHOLD=5.0`，与实验记录一致）；相对提升 `(C−B)/B` 作为次要指标与 `gain_pp` **双录**于评测报告，不参与启用判定（避免「5%」在相对/绝对间的歧义）。
...
- 召回率：纯向量 < 混合 < QUG 增强，逐级可量化；**QUG 无显著增益（绝对 `gain_pp ≥ 5.0pp`，见 §5.2 退出条件）则默认关闭，不作为验收铁律**
...
5. QUG 五类边构建 + golden-queries 评测（纯向量 vs 混合 vs QUG，含退出条件判定）✅（Step 5 已交付，2026-09-23；含 0004 迁移（qug_builds 代次父表 + qug_page_snapshots/qug_intent_edges 独立持久化）、BLAKE3 source_hash 与 hash 命中复用、单事务原子发布（失败回滚旧图可读）、启动加载 hash 校验与 stale/disabled 显式 fallback、golden 扩容至 134 条（legacy 34 + 新增 100，配额 synonym 25/intent 20/negation 20/attribute_filter 20/negative 15）、A/B/C 三档评测（recall@1/5/10 + negative_precision + 按 kind 分层，双语 + JSON 报告）、`wiktor qug build`/`wiktor eval` CLI（退出码 0/1/2/3/4）；退出条件判定：milk-tea fixture 离线评测（`--no-qdrant` mock 向量后端）C 相对 B 的 recall@10 增益 +40.17pp ≥ 5pp → `qug_decision=enabled`，真实向量后端接入后需复测；**真实后端复测（2026-09-24，qwen3.8-max 编译 72/120 accepted + qwen3.7-text-embedding-flash 1024 维 + qdrant，见 `docs/design/real-backend-experiment.md`）确认 enabled：A 纯FTS recall@10=0.446 / B 混合=0.889（真实嵌入 vs 纯FTS +44.3pp）/ C QUG=0.962（C−B +7.26pp ≥ 5pp）**，intent/negation 两类从 0 命中提升到 0.95/1.0；实现偏差见 `docs/design/step5-qug-build.md` §9）
6. 反馈分析器 + `POST /feedback`（认证 / 限流 / 幂等）✅（Step 6 已交付，2026-09-23；含 0005 迁移（feedback_events/review_queue/feedback_rejections 三表 + query_logs 补 domain 与滤空/放宽重试三列及部分索引）、`wiktor-feedback`（FeedbackStore trait + 标准分析器：零召回/低质召回/改写失败三信号确定性聚合、subject_json 确定性规则与 BLAKE3 report_hash、双语 Markdown + JSON 报告原子落盘）与 `wiktor-server` 新 crate（axum 0.7：`POST /feedback` + `GET /health` + `GET /metrics`，Bearer API key 域授权不落库、固定窗口限流 60s/120、body 每事件幂等键（`(domain,idempotency_key)` 唯一，重复请求幂等回放）、64KiB/100 条输入预算超限 413 并计隔离拒绝表）、QueryEngine 空候选放宽重试恰一次（FilterRelaxer）与 log_query 返回 log_id、审核队列 approve 在同一 BEGIN IMMEDIATE 内调用 admit_compile（查询模板批准只写审计）、`wiktor feedback analyze|list|review` CLI（退出码 0/1/2/3/4）、WAL + busy_timeout=5000 收口与双写并发 smoke；离线验收 289 测试全绿 + clippy/fmt 干净；实现偏差见 `docs/design/step6-feedback-loop.md` §12）
7. `wiktor-server`（gRPC + HTTP）✅（Step 7 已交付，2026-09-23；含 `proto/wiktor.proto` 六 service（Search/Compile/QugBuild/Review/Compatibility/Status，tonic 0.14 + tonic-prost-build 生成）、方法级 API key 授权（`WIKTOR_API_KEYS` 新 JSON `{"domain":{"secret","methods"}}`，旧 `WIKTOR_FEEDBACK_API_KEYS` 兼容为 feedback-only，gRPC interceptor 与 HTTP middleware 同源同语义、BLAKE3 标签不落日志）、统一 gRPC/HTTP 错误映射（16 未认证 / 7 无权限 / NOT_FOUND / invalid_argument）、gRPC Search + HTTP `GET /search`（QueryEngine 装配、Mock 向量 + 确定性嵌入离线验收、预算校验）、Status（schema + row_counts）、QugBuild（build_and_publish_qug_from_bytes）、Compatibility（只读 check）、Compile 异步任务模型（`Compile.Admit` 只 admission 不 claim（core 新增 `PipelineExecutor::admit_batch` 复用 admission 语义）、`CompileWorker` 常驻消费（core 新增 `process_claimed_task` 窄接口复用 process_lease 全流程，启动 recover + 周期 LeaseReaper 30s + cancel drain；worker 与 Admit 同源装配保证 content_hash 复现；WIKTOR_COMPILE_WORKERS 1..=8）、`Compile.Status`（kernel `compile_task_status` 快照））、Review（List/Approve/Ignore 复用 kernel 事务、reviewer 取认证 label 客户端不可传）、CLI `wiktor serve --db --listen-http --listen-grpc --domain`（feature server 转发）+ 旧 `wiktor-server` bin 委托共享 `run_server` 双监听同进程；离线验收 391 测试全绿 + clippy/fmt 干净；实现偏差见 `docs/design/step7-server-grpc.md` §9（STEP7-001..010））
8. 一致性仲裁维度 + 任务状态机补全（租约 / 死信 / 兼容检查）✅（Step 8 已交付，2026-09-23；含 0006 迁移（compile_tasks 增 consistency_status/compatibility_status 两列 + compile_attempts 增 consistency_json/compatibility_json 诊断列 + review_queue 表重建放宽 action CHECK 至六动作并保留 FK/UNIQUE + idx_compile_tasks_dead_review/idx_compile_tasks_preflight/idx_review_task 部分索引 + down 三守卫：任一 Step8 审核行/非默认状态/非空诊断 JSON 即拒回退）、consistency.rs（ConsistencyArbiter trait + 默认 SourceRefConsistencyArbiter：仅比较 compare_pointers 声明的 RFC6901 source-ref 证据、canonical JSON 分组比较、BLAKE3 摘要不落原文、compared_claims=0 返回 None、五维等权 overall 与 min_consistency 门槛、FTS top-k provider 有界召回 top_k=8 上限 32 且 bm25/page_id 稳定排序绝不全量）、compatibility.rs（CompatibilitySpec semver 矩阵 + 全量扫描 accepted 页与任务快照的 domain_pack/schema/prompt/artifact 版本、损坏 JSON 计 violation、缺失矩阵 MISSING_COMPATIBILITY_MATRIX 告警、只读 domain check 不写库）、lease.rs（LeaseReaper 周期回收 30s + 关闭 drain）、executor（每 lease 心跳 loop 消费 heartbeat_seconds、首个非空批次 admission 前 preflight fail-closed、dead 分支死信+一致性冲突同事务入队、head 快照守卫重放新 epoch）、审核（compile_dead_letter 按 task_id 幂等去重、approve 以原任务快照 force 新 epoch 同事务回填、consistency_conflict 只能转换为一次 supplemental_compile、compatibility_conflict 仅审计批准）、CLI（`wiktor domain check` --json 只读退出码 3、compile 自动 preflight、feedback review approve/ignore 新动作 fail-closed）；离线验收 374 测试全绿 + clippy/fmt 干净；实现偏差见 `docs/design/step8-consistency-state-machine.md` §11（STEP8-001..036，中英同步））
9. HA：litestream 主从 → 远期 Raft（rqlite 模式）✅（Step 9 已交付，2026-09-24；单机 + 远端/本地副本持续 WAL 复制（litestream 0.5.17 固定版，Linux Debian 13 真实部署），`deploy/` 落地安装/前置检查/隔离恢复演练脚本；零 Rust 改动（D10）；真实恢复演练在 Linux disposable 库上 `PASS`——integrity/外键/行数/marker 全过（A1/A2/A7/A8/A9 实测），SFTP 到 Mac 与 systemd 常驻为生产 bring-up 项；实现偏差见 `docs/design/step9-backup-ha.md` §7（STEP9-011..015：资产名无 v 前缀与 x86_64、v0.5.17 移除 generations 用 status/ltx、sync-interval 为 db 级、离线复用与可移植 sha256））
10. 集群分片 + 插件生态（Meilisearch 出口、外部向量库）+ 第二官方领域包（技术文档）✅（Step 10 已交付，2026-09-24；**实装**：①第二官方领域包 `examples/tech-docs/`（20 知识页 + 120 事实 + 134 golden，证明领域包插件点对非电商域可扩展）+ 领域包贡献指南 `docs/domain-pack-guide(.en).md`；②golden 过滤器泛化（`GoldenFilters` 由 milk-tea 硬编码 price/sugar/size/ingredients 改为领域通用条件列表 `{"conditions":[...]}`，`golden_filters_to_filters` 逐条透传字段名，134 条 milk-tea golden 机械迁移保持 134/配额/legacy 断言，tech-docs eval 冒烟绿）；③qdrant 向量后端拆为独立插件 crate `wiktor-vector-qdrant`（只依赖 core，`cargo tree` 证伪 server/feedback；core 去掉 vector-qdrant feature 与 qdrant-client 依赖，MockVectorStore 留 core 作评测基线，CLI 装配插件）；④Meilisearch 出口插件 `wiktor-adapter-meilisearch` + `wiktor export meilisearch` 子命令（feature export-meilisearch，accepted 页 PATCH 到 `${WIKTOR_MEILISEARCH_URL}`，mock HTTP 验证 2/2，quarantine 不导出）；**集群分片 = 设计规划**（`docs/design/step10-cluster-sharding(.en).md`：domain 一级分片键 + 路由 + 整 domain 重平衡 + 单写/同域原子发布边界，与远期 rqlite-Raft 衔接，明确不建 crate/不实现）；workspace 393+ 测试全绿；实现偏差见 `docs/design/step10-plugin-ecosystem.md` §5（STEP10-001..003：GoldenFilters 用 `{"conditions":[...]}` 而非裸数组、core error.rs 增 `Error::External` 破 server/feedback 穷尽 match、core 默认 feature 由 `["vector-qdrant"]` 降为空以让 CLI 装配插件））
...
12. TUI（补 Step11 欠账）+ console 前端真实数据 + 生产部署编排 ✅（Step 12 已交付，2026-09-25；①`wiktor tui`（feature `tui`，ratatui 0.29 + crossterm 0.28）：四 Tab（仪表盘/任务/审阅/查询）与 Web console 同源数据面（in-process kernel 读 + QueryEngine 混合 fallback），state 纯函数 + TestBackend 测试，键盘导航（1-4/Tab/Enter/q）；②console 前端接真实数据：`/api/overview` 增 due 任务状态计数，`code.html` 挂 `data-live` 标记 + vanilla fetch 渲染层（15s 轮询、offline 徽标回退、检索框接 `POST /api/search` 渲染 hits 与 fts/vector/rrf_k 分路诊断），浏览器实测真实 seed 数据渲染 PASS；③生产部署编排 `deploy/`：`wiktor-server`/`wiktor-console` systemd 单元（沙箱对齐 litestream，loopback 监听）+ `wiktor.env.example`（WIKTOR_API_KEYS）+ `install-wiktor.sh`（服务器 release 构建，幂等）+ `smoke-deploy.sh` + bring-up runbook，Linux Debian 13 实机部署验收；前置：rsproxy 镜像解锁 crates.io（STEP11-001/005 根因解除）；偏差见 `docs/design/step12-tui-console-prod(.en).md` §7（STEP12-001..005；005=用户指令下 code.html 由参考原型重写为纯真实数据监督 UI——仅渲染 /api/* 真实数据零 mock、英文默认可切中文、单文件零外部依赖））

13. 生产就绪（serve 真实检索 + 备份副本 + 可观测/CI + 反馈闭环 E2E）✅（Step 13 已交付，2026-09-25；①serve 真实检索：`wiktor serve` 由 CLI 装配者按 env 注入向量后端/嵌入器/QUG（`WIKTOR_VECTOR_BACKEND=mock|qdrant` + `WIKTOR_QDRANT_URL/API_KEY` + `WIKTOR_EMBEDDING_*`，缺省 mock 离线可跑；维度探测装配），HTTP `GET /search` 切换到与 gRPC 共享的 `QueryEngine<dyn VectorStore>`——响应形状不变，diagnostics/log_id 由占位变真实并落 query_logs；②console 占位清偿：kernel 只读 `quality_summary()`/`list_domains()` + `/api/quality` `/api/domains` 真实实现 + 概览页质量面板；③备份副本：litestream file:// 副本接生产（`/srv/backup/wiktor/litestream`，sync-interval 1s，status ok）+ 生产库隔离恢复演练 PASS（integrity/fk/逐表行数对账一致）；④可观测 + CI：Prometheus 抓 `/metrics`（target up）+ `up==0` 告警规则 + GitHub Actions（fmt/clippy -D warnings/test 含 tui feature）；⑤反馈闭环 E2E：`deploy/smoke-feedback.sh` 本机与生产全 PASS——search（log_id）→ POST /feedback → `feedback analyze --domain-pack`（zero_recall 盲点 + 确定性实体增强）→ review approve → worker 编译发布 → 复检命中 0→1；偏差见 `docs/design/step13-production-ready(.en).md` §7（STEP13-001..003；001=analyze 可选实体增强桥接 step6 spec 的报告形 subject 与 approve 五字段校验缺口））
...
7. **评测交付物补全**：golden-queries ≥ 100 条 + 50-100 页人工标注集；QUG 增设退出条件（绝对 `gain_pp ≥ 5.0pp` 默认关闭，口径见 §5.2）。
